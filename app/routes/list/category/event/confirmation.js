...
import {
  getDataToSubmit,
  setDataToSubmit,
} from 'kursausschreibung/framework/storage';
import {
  postPerson,
  putPerson,
  postAddress,
  postSubscription,
  postSubscriptionDetailsFiles,
  ...
  model() {
    ...
    return autoCheckForLogin()
      ...
      .then(() => {
        ...
        if (personId === 0) {
          promises.push(
            createAddresses(useCompanyAddress, addressData, companyAddressData)
          );
        } else {
          ...
          return promise.then((id) => {
            ...
            if (additionalPeople.length > 0) {
              subscriptionData.SubscriptionDetails.push({
                VssId: SUBSCRIPTION_DETAIL_ALLOW_MULTIPLE_PEOPLE,
...
function createAddresses(useCompanyAddress, addressData, companyAddressData) {
  ...
  return createPerson(addressData)
    .then((id) => {
      personId = id;
      ...
      companyAddressData.Country =
        companyAddressData.Country === null
          ? 'Schweiz'
          : companyAddressData.Country;
      companyAddressData.CountryId =
        companyAddressData.CountryId === null
          ? 'CH'
          : companyAddressData.CountryId;
...
function createPerson(addressData) {
  ...
  if (settings.personDefaultValue instanceof Object) {
    Object.keys(settings.personDefaultValue).forEach((key) => {
  ...
  return postPerson(addressData).then((response) => {
    ...
    if (duplicateHeader === null && locationHeader === null) {
      throw new Error(
        'failed to read personId. neither x-duplicate nor location header could be read.'
      );
    }
