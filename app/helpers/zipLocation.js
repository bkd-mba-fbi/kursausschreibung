export function getZipLocation(){
    let inputZips = document.getElementsByName('Zip');
    inputZips.forEach(function(element, index) {
        let searchPattern = element.value; 
        let oldlocation =  document.getElementsByName('Location')[index].value;

        if(oldlocation === '') {
            document.getElementsByName('Location')[index].value = searchZipLocation(searchPattern);
        }
...
function searchZipLocation(searchString) {
    let value = '';
    
        zipLocation().forEach(element => {
            if(value === '') {
                if (element.Zip === searchString) {
                    value =  element.Location;
                } else if (element.Location === searchString) {
                    value = element.Location;
                }
            }
            });
