...
var api = {
  ...
  updateGradingItem: function (model, success) {
    api.put('GradingItems/' + model.IdSubscription + '_' + model.IdPerson, model, success);
  },
  ...
  init: function () {
    ...
    $.ajaxSetup({
      headers: {
        'CLX-Authorization': 'token_type=' + appConfig.tokenType + ', access_token=' + that.getLoginToken()
      }
    });
    $.support.cors = true;
  },
  ...
  postFileBuffer: function (relativeUrl, buffer, success, error, ignoreToken) {
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        api.handleError('POST', appConfig.apiUrl + '/' + relativeUrl, error, jqXHR, textStatus, errorThrown);
      }
    });
  },
  ...
  call: function (method, relativeUrl, data, success, error, ignoreToken, headers) {
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        api.handleError(method, url, error, jqXHR, textStatus, errorThrown);
      },
      headers: headers
    });
  },
  ...
  handleError: function (method, url, error, jqXHR, textStatus, errorThrown) {
    ...
    if (jqXHR.status === 409) {
      if (doInstrumentation)
        instrument('validationErrorOccurred', jqXHR);
    } else {
      storage.errorInfo({
