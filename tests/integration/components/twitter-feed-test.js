...
module('Integration | Component | twitter-feed', function(hooks) {
  ...
  test('renders a timeline link for the provided username', async function(assert) {
    ...
    assert.equal(link.getAttribute('href'), 'https://twitter.com/emberjs', 'uses the username in href');
    assert.ok(link.textContent.includes('Tweets by emberjs'), 'link text includes username');
  });
