...
module('Integration | Component | event-details-table', function(hooks) {
  ...
  test('renders detail rows from configured event fields', async function(assert) {
    ...
    assert.ok(this.element.textContent.includes('Max Muster'), 'leadership value is rendered');
    assert.ok(this.element.textContent.includes('120 CHF'), 'price value is rendered');
  });
  ...
  test('renders event text rows when event texts are present', async function(assert) {
    ...
    assert.ok(this.element.textContent.includes('Wichtig'), 'text label is rendered');
    assert.ok(this.element.textContent.includes('Unterlagen mitbringen'), 'text memo is rendered');
  });
});
